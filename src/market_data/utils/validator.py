...
This module provides functionality to validate the structure and integrity of historical market
data for financial symbols. It ensures the presence of required columns, correct datetime ordering
and valid symbol configurations.
"""
...
from market_data.transform.enriched_data import EnrichedData
...
class Validator:
    ...
    @staticmethod
    def _has_invalid_prices(df: pd.DataFrame, raw_flow: bool) -> Union[str, None]:
        ...
        if result is None:
            if raw_flow is True:
                ...
                if len(negative_col) > 0:
                    result = f"Non-positive value price found. Columns: {negative_col}"
            else:
                for col in ["open", "low", "high", "close", "adj_close"]:
                    if (df[col] < 0).any():
                        negative_col.append(col)
                if len(negative_col) > 0:
                    result = f"Negative value price found. Columns: {negative_col}"
        return result
    ...
    def _basic_checks(
        ...
        price_error = Validator._has_invalid_prices(df, raw_flow)
        if price_error:
            return price_error
    ...
    @staticmethod
    def validate_data(raw_flow: bool) -> bool:
        ...
        symbols_data = (
            RawData.load().get("symbols")
            if raw_flow
            else EnrichedData.load().get("symbols")
        )
