...
class TechnicalIndicators:
    ...
    def compute_candle_pattern(
        open_: pd.Series, high: pd.Series, low: pd.Series, close: pd.Series, raw: bool
    ) -> Optional[pd.Series]:
        """
        ...
        if raw is True:
            return pd.Series([c.detect_pattern() for c in candles], index=open_.index)
        return pd.Series([c.score() for c in candles], index=open_.index)
    ...
    def compute_multi_candle_pattern(
        ...
        candles = [Candle(o, h, l, c) for o, h, l, c in zip(open_, high, low, close)]
        if raw is True:
            results_raw: list[str | None] = [None] * len(candles)
            for i in range(2, len(candles)):
                window = candles[i - 2 : i + 1]
                results_raw[i] = MultiCandlePattern.detect_pattern(window)
            return pd.Series(results_raw, index=open_.index, dtype="object")
        results: list[float] = [np.nan] * len(candles)
        for i in range(2, len(candles)):
            window = candles[i - 2 : i + 1]
            results[i] = MultiCandlePattern.score(window)
        return pd.Series(results, index=open_.index)
...
class EnrichedData:
    ...
    def _add_indicators(df: pd.DataFrame, prefix: str = "") -> pd.DataFrame:
        ...
        df[prefixed("candle_pattern")] = TechnicalIndicators.compute_candle_pattern(
            df["open"], df["high"], df["low"], df["close"], len(prefix) == 0
        )
