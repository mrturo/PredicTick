...
class EnrichedData:
    ...
    @staticmethod
    def _resample_df_by_ratio(df: pd.DataFrame, enriched_interval: str) -> pd.DataFrame:
        """Dynamically regroup a DataFrame based on the ratio between two intervals."""
        df = df.sort_values("datetime").copy()
        df["datetime"] = pd.to_datetime(df["datetime"], utc=True)

        enriched_minutes = IntervalConverter.to_minutes(enriched_interval)
        group_offset = pd.tseries.frequencies.to_offset(f"{enriched_minutes}min")
        df["group_time"] = df["datetime"].dt.floor(group_offset)
        df = df.set_index("group_time")

        idx_open = df.groupby(level=0)["datetime"].idxmin()
        idx_close = df.groupby(level=0)["datetime"].idxmax()

        df_open = df.loc[idx_open]
        df_close = df.loc[idx_close]
        grouped = df.groupby(level=0)

        return pd.DataFrame(
            {
                "datetime": df_open["datetime"],
                "open": df_open["open"],
                "close": df_close["close"],
                "low": grouped["low"].min(),
                "high": grouped["high"].max(),
                "volume": grouped["volume"].sum(),
                "adj_close": df_close["adj_close"],
            }
    ...
    def process_symbol(key: str, value: Any, ranges: Any, interval: Any) -> dict:
        ...
        if interval_raw_data != interval_enriched_data:
            ...
            df = EnrichedData._resample_df_by_ratio(df, interval_enriched_data)
