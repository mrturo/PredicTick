...
class EnrichedData:
    ...
    @staticmethod
    def _add_indicators(
        df: pd.DataFrame, raw_df: pd.DataFrame, prefix: str = ""
    ) -> pd.DataFrame:
        """
        ...
        df[prefixed("candle_pattern_1h")] = TechnicalIndicators.compute_candle_pattern(
            raw_df,
            EnrichedData._RAW_DATA_INTERVAL,
            df["datetime"],
            len(prefix) == 0,
            "1h",
        )
    ...
    def _compute_features(
        ...
        df = EnrichedData._add_indicators(df, raw_df)
        df = EnrichedData._add_indicators(df, raw_df, prefix="scaled_")

        always_keep = {
            "volume", "obv", "relative_volume",
            "volume_change", "volume_rvol_20d",
        }
        columns_to_check = [c for c in df.columns if c not in always_keep]
        columns_with_data = [
            c for c in columns_to_check if not df[c].isna().all()
        ]
        if not columns_with_data:
            return df.iloc[0:0]

        last_nan_idx = (
            df[columns_with_data]
            .isna()
            .any(axis=1)
    ...
    def process_symbol(key: str, value: Any, ranges: Any, interval: Any) -> dict:
        ...
        df = EnrichedData._compute_features(resampled_ratio_df, raw_df, ranges)
