...
A lightweight stub :pyfunc:`compute_internal_multi_candle_pattern` is kept to
preserve backward compatibility with legacy pipelines that expected its
presence; the actual implementation lives elsewhere in the codebase.
"""
...
from market_data.processing.candles.candle import Candle
from market_data.processing.candles.multi_candle_pattern import MultiCandlePattern
...
def compute_internal_multi_candle_pattern(  # noqa: D401, WPS110
    _raw_df: pd.DataFrame,
    _enriched_df: pd.DataFrame,
    output_as_name: bool = True,
) -> Optional[pd.Series]:
    """Stub kept for backward compat – *real* logic implemented elsewhere."""
    try:
        if output_as_name:
            return pd.Series(pd.NA, dtype="category")
        return pd.Series(np.nan, dtype="float32")
    except Exception as exc:  # noqa: BLE001  # pylint: disable=broad-exception-caught
        Logger.warning(f"[compute_internal_multi_candle_pattern] failure: {exc}")
        if output_as_name:
            return pd.Series(pd.NA, dtype="category")
        return pd.Series(np.nan, dtype="float32")
