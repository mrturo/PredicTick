...
from market_data.processing.enrichment.indicator_builder import IndicatorBuilder
from market_data.processing.enrichment.market_context import MarketContext
from market_data.processing.resampling.time_resampler import TimeResampler
...
class EnrichedData:
    ...
    def _compute_features(
        ...
        enriched_df, market_time = IndicatorBuilder.add_indicators(
            raw_df, enriched_df, market_context
        )
        enriched_df, _ = IndicatorBuilder.add_indicators(
            raw_df, enriched_df, market_context, prefix="scaled_"
        )
    ...
    def _process_symbol(
        ...
        df, market_time = EnrichedData._compute_features(
            raw_df, resampled_ratio_df, ranges, market_context
        )
