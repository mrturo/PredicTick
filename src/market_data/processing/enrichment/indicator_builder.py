...
from market_data.processing.enrichment.market_context import MarketContext
from market_data.processing.indicators.patterns import (
    compute_candle_pattern,
    compute_internal_multi_candle_pattern,
    compute_multi_candle_pattern,
)
from market_data.processing.indicators.price import (
...
from market_data.processing.indicators.schedule import compute_market_time
from market_data.processing.indicators.temporal import (
...
from market_data.processing.indicators.trend import (
    compute_adx_14d,
    compute_atr,
    compute_atr_14d,
...
from market_data.processing.indicators.volume import (
    compute_obv,
...
class IndicatorBuilder:  # pylint: disable=too-few-public-methods
    ...
    def add_indicators(
        ...
        enriched_df = IndicatorBuilder._add_technical_indicators(enriched_df, prefixed)
        ...
        enriched_df, market_time = IndicatorBuilder._add_market_time_indicators(
            raw_df, enriched_df, market_context, prefixed, is_raw
        )
        return enriched_df, market_time

    @staticmethod
    def _add_technical_indicators(enriched_df: pd.DataFrame, prefixed) -> pd.DataFrame:
    ...
    def _add_market_time_indicators(
        ...
        enriched_df[prefixed("internal_multi_candle_pattern")] = (
            compute_internal_multi_candle_pattern(raw_df, enriched_df, is_raw)
        )
