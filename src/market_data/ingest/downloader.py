...
from market_data.ingest.provider import PriceDataConfig, Provider, TickerMetadata
from market_data.utils.interval import Interval
from market_data.utils.timezone_normalizer import TimezoneNormalizer
...
class Downloader:
    ...
    def get_historical_prices(
        ...
        if (
            Downloader._RAW_DATA_INTERVAL is None
            or len(Downloader._RAW_DATA_INTERVAL.strip()) == 0
        ):
            raise ValueError(
                "Interval parameter is not defined. Please set it before proceeding."
            )
        if (
            Downloader._HISTORICAL_DAYS_FALLBACK[Downloader._RAW_DATA_INTERVAL.strip()]
            is None
        ):
            raise ValueError(
                f"Days fallback parameter is not defined for the interval "
                f"'{Downloader._RAW_DATA_INTERVAL.strip()}'. Please set it before proceeding."
            )
        days_fallback = Downloader._HISTORICAL_DAYS_FALLBACK[
            Downloader._RAW_DATA_INTERVAL.strip()
        ]
        ...
        start = min(
            ...
            (
                ...
                if last_datetime
                else now - timedelta(days=days_fallback)
            ),
