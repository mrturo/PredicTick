...
from market_data.utils.interval import Interval
...
class RawData:
    ...
    @staticmethod
    def set_last_update(last_updated: Optional[pd.Timestamp]) -> None:
    ...
    def normalize_historical_prices(
        ...
        Args:
            symbols (List[Dict[str, Any]]): Raw market symbol data.

        Returns:
            Dict[str, Dict[str, Any]]: Dictionary indexed by symbol with normalized entries.
        """
        normalized = {}
    ...
    def load(filepath: Optional[str] = None) -> Dict:
        ...
        if raw_id is None or len(raw_id.strip()) == 0:
            RawData.set_new_id()
            RawData._last_updated = now
        else:
            RawData._id = raw_id.strip()
            RawData._last_updated = (
        ...
        return {
            "id": RawData._id,
            "last_updated": RawData._last_updated,
            "last_check": RawData._last_check,
            "interval": RawData._interval,
            "symbols": RawData._symbols,
        }
    ...
    def save(filepath: Optional[str] = None) -> dict[str, Any]:
        ...
        result = {
            ...
            "interval": RawData._interval,
            "symbols": list(RawData._symbols.values()),
        }
    ...
    def _update_stale_symbols() -> None:
        ...
        latest_date = RawData._latest_price_date or pd.Timestamp.now(tz="UTC")
    ...
    def _detect_stale_symbols(
        ...
        stale = []
        for symbol, entry in RawData._symbols.items():
            datetimes = [
