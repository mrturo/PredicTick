...
from utils.interval import IntervalConverter
...
class TimeResampler:
    ...
    def by_ratio(
        ...
        from_interval: str,
        to_interval: str,
        datetime: Optional[pd.Series] = None,
    ) -> pd.DataFrame:
        """Aggregate a DataFrame of OHLCV candles to a lower-frequency interval.

        Args:
            from_df (pd.DataFrame): Input DataFrame sorted or sortable by datetime column.
                Must include the following columns: 'datetime', 'open', 'high', 'low',
                'close', 'volume', 'adj_close'.
            from_interval (str): String representation of the current interval
                (e.g., '1m', '5m', '15m').
            to_interval (str): String representation of the target lower-frequency interval
                (e.g., '1h', '4h', '1d'). Must be divisible by `from_interval`.
            datetime (Optional[pd.Series]): Optional anchor series for defining window alignment.
                If provided, its latest value is used as the anchor point.

        Returns:
            pd.DataFrame: Aggregated OHLCV data at the new lower-frequency interval.

        Raises:
            ValueError: If the `to_interval` is not divisible by `from_interval`.
        """
        ...
        windows = TimeResampler._generate_windows(df, window_delta, datetime)
    ...
    def _to_utc_datetime_series(
        obj: Union[None, pd.Series, pd.Index, Iterable, datetime, int, float, np.number, str]
    ) -> pd.Series:
        """
        Helper: convert *obj* to a tz-aware Series[datetime64[ns, UTC]] with NaTs
        removed.  Returns an **empty** Series if the input is None or all values
        are invalid.
        """
        if obj is None:
    ...
    def _normalize_window_delta(delta: Union[timedelta, np.timedelta64, int, float]) -> timedelta:
        """
    ...
    def _generate_windows(
        ...
        window_delta : timedelta | np.timedelta64 | int | float
            Length of each aggregation window.  Numeric values are treated as
            **seconds**.
        anchor : various or None
            Alignment reference; only its **latest** valid entry is used.

        Returns
        -------
        list[pandas.Timestamp]
            At least one timestamp, tz-aware (UTC).
        """
        # ── Normalise inputs ─────────────────────────────────────────────────────────
        ...
        # ── Branch: no anchor, build a simple range ─────────────────────────────────
        else:
            windows = list(
    ...
    def _aggregate_window(
        ...
        Args:
            df (pd.DataFrame): Source DataFrame with OHLCV data.
            start (pd.Timestamp): Start of the window (inclusive).
            end (pd.Timestamp): End of the window (exclusive).

        Returns:
            Optional[dict]: Dictionary containing the aggregated OHLCV data for the window,
            or None if the window contains no data.
        """
