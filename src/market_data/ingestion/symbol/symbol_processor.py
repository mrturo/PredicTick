...
from market_data.ingestion.downloaders.downloader import Downloader
...
class SymbolProcessor:
    ...
    def _resolve_exchange_id(entry: Optional[dict[str, Any]]) -> str:  # noqa: D401
        ...
        ----------
        entry
            Raw symbol metadata as returned by :pyfunc:`RawData.get_symbol`.

        Returns
        -------
        str
            Sanitised exchange ticker (always non‑empty).

        Raises
        ------
        ValueError
            If neither *entry* nor the class‑level fallback contain a valid
            exchange identifier.
        """
