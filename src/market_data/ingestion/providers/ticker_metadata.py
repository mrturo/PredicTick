...
class TickerMetadata:  # pylint: disable=too-many-instance-attributes
    ...
    def from_dict(data: Dict[str, Any]) -> TickerMetadata:
        """Builds TickerMetadata from a dict using camelCase → snake_case conversion."""
        global _TYPE_HINTS  # noqa: PLW0603
        if not _TYPE_HINTS:
            _TYPE_HINTS = get_type_hints(TickerMetadata)
        filtered_data: Dict[str, Any] = {}
        for f in fields(TickerMetadata):
            camel_name = TickerMetadata._snake_to_camel(f.name)
            val = data.get(camel_name, None)
            typ = _TYPE_HINTS.get(f.name, f.type)
            filtered_data[f.name] = TickerMetadata._parse_value(val, typ)
