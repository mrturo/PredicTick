...
class Downloader:
    ...
    _INTERVAL: Optional[str] = _PARAMS.get("interval")
    ...
    def is_symbol_available(self, symbol: str) -> bool:
        ...
        try:
            ...
            df = self.get_price_data(symbol, start, now, Downloader._INTERVAL)
            return not df.empty
    ...
    def get_historical_prices(
        ...
        if Downloader._INTERVAL is None or len(Downloader._INTERVAL.strip()) == 0:
            raise ValueError(
                "Interval parameter is not defined. Please set it before proceeding."
            )
        if Downloader._HISTORICAL_DAYS_FALLBACK[Downloader._INTERVAL.strip()] is None:
            raise ValueError(
                f"Days fallback parameter is not defined for the interval "
                f"'{Downloader._INTERVAL.strip()}'. Please set it before proceeding."
            )
        days_fallback = Downloader._HISTORICAL_DAYS_FALLBACK[
            Downloader._INTERVAL.strip()
        ]
        ...
        while current_start < end:
            ...
            df = self.get_price_data(
                symbol, current_start, current_end, Downloader._INTERVAL
            )
            if not df.empty:
