...
from _market_data.downloader import Downloader
from _market_data.enrichment import Enrichment
...
class SymbolProcessor:
    ...
    @staticmethod
    def _process_single_symbol(symbol: str, entry: Optional[dict]) -> tuple:
        ...
        if not existing_df.empty and "datetime" in existing_df.columns:
            ...
            if isinstance(last_dt_ts, pd.Series):
                last_dt = last_dt_ts.dropna().max()
            else:
                last_dt = last_dt_ts if pd.notnull(last_dt_ts) else None
        else:
            last_dt = None
