...
class Validator:
    ...
    def _has_missing_columns(df: pd.DataFrame) -> Union[str, None]:
        required_market_columns: list[str] = list(
            set(Validator._REQUIRED_MARKET_RAW_COLUMNS)
            | set(Validator._REQUIRED_MARKET_ENRICHED_COLUMNS)
        )
    ...
    def _clean_extreme_z_scores(df: pd.DataFrame) -> Tuple[pd.DataFrame, bool]:
        ...
        if isinstance(returns, pd.Series):
            ...
            returns_clean = pd.Series(dtype=float)
        ...
        if len(returns_clean) > 5:
            ...
            if isinstance(extreme_z, pd.Series):
                extreme_z_idx = extreme_z.index
            elif extreme_z is not None and not pd.isna(extreme_z):
                extreme_z_idx = pd.Index([extreme_z])
            else:
                extreme_z_idx = pd.Index([])
            df, changed = Validator._set_nan_if_not_empty(df, extreme_z_idx, "return")
