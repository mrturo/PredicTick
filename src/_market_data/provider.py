...
class TickerMetadata:
    ...
    def from_dict(data: Dict[str, Any]) -> TickerMetadata:
        ...
        Args:
            data (Dict[str, Any]): Raw metadata dictionary with camelCase keys.
        Returns:
            TickerMetadata: Populated instance.
        """
        ...
        def parse_value(val: Any, typ: Any) -> Any:
            """
            ...
            Returns:
                Any: Converted value or None.
            """
            if val is None or (isinstance(val, str) and val.strip() == ""):
                return None

            typ = _extract_real_type(typ)

            try:
                return _convert_value(val, typ)
            except Exception:  # pylint: disable=broad-exception-caught
                return None
        ...
        def _extract_real_type(typ: Any) -> Any:
            ...
            Args:
                typ (Any): The original type (possibly a Union).

            Returns:
                Any: The concrete type without None.
            """
...
class Provider:
    ...
    def get_metadata(self, symbol: str) -> TickerMetadata:
        ...
        Returns:
            TickerMetadata: Metadata object.
        """
    ...
    def get_price_data(self, config: PriceDataConfig) -> Optional[pd.DataFrame]:
        ...
        Args:
            config (PriceDataConfig): Configuration object for data download.
        Returns:
            Optional[pd.DataFrame]: Price data as a DataFrame, or None on failure.
        """
        if config.proxy:
