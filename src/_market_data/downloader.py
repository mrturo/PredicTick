...
from _market_data.provider import PriceDataConfig, Provider, TickerMetadata
...
class Downloader:
    ...
    def is_symbol_available(self, symbol: str) -> bool:
        ...
        try:
            ...
            df = self.get_price_data(symbol, start, now, self._RAW_DATA_INTERVAL)
            return not df.empty
    ...
    def get_historical_prices(
        ...
        while current_start < end:
            ...
            df = self.get_price_data(
                symbol, current_start, current_end, self._RAW_DATA_INTERVAL
            )
            if not df.empty:
