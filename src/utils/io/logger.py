...
Provides standardized, timestamped output for log messages.
"""
...
class Logger:
    ...
    log_basepath = PathUtils.build("logs")
    ...
    def _initialize():
        ...
        if Logger.logger is not None:
            return
        Logger.LOGS_DIR = (os.getenv("LOGS_DIR", "True") or "").strip().lower() in (
            "true",
            "",
        )
        ...
        if not is_pytest and Logger.LOGS_DIR:  # pragma: no cover
    ...
    def log(symbol: str, message: str) -> str:
        ...
        Logger.logger.info(formatted)  # type: ignore
        return formatted
