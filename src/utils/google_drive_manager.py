...
class GoogleDriveManager:
    ...
    def __init__(
        self, service: Optional[Resource] = None, drive_folder_id: Optional[str] = None
    ) -> None:
    ...
    def _authenticate(self) -> Credentials:
        ...
        try:
            ...
            error_msg = str(ex)
            Logger.warning(f"Google Drive authentication failed: {error_msg}")
            creds = None
    ...
    def _validate_filename(
        local_filepath: Optional[str], drive_filename: Optional[str] = None
    ) -> Tuple[str, str]:
    ...
    def upload_file(
        self, filepath: Optional[str], backup: Optional[bool] = False
    ) -> None:
        ...
        try:
            ...
            if file_id:
                ...
                if (
                    isinstance(self._drive_folder_id, str)
                    and self._drive_folder_id.strip()
                ):
                    file_metadata["parents"] = [self._drive_folder_id.strip()]  # type: ignore
                media = MediaFileUpload(filepath, mimetype=mime_type)
                created_file = (
                    self._service.files()
    ...
    def _get_existing_file_id(self, filename: str) -> Optional[str]:
        ...
        if self._service is None:
            Logger.warning(
                "Google Drive service is not available. Cannot get existing file ID."
            )
            return None
    ...
    def _create_backup(
        self, filepath: str, drive_filename: str, mime_type: Optional[str]
    ) -> None:
        ...
        backup_metadata: dict[str, Any] = {"name": backup_name}
        if self._drive_folder_id:
        ...
        if self._service is None:
            Logger.warning(
                "Google Drive service is not available. Cannot create backup file."
            )
            return
    ...
    def download_file(self, filepath: str, download_path: Optional[str] = None) -> bool:
