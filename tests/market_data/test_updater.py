"""
Unit tests for the Updater class in the update module.
...
from market_data.ingest.ingester import Ingester
...
def test_is_valid_marketdata_valid():
    ...
    result = Ingester.is_valid_marketdata(valid_data)
    if not result:
...
def test_is_valid_marketdata_invalid():
    ...
    result = Ingester.is_valid_marketdata(invalid_data)
    if result:
        raise AssertionError("Expected invalid marketdata structure to return False")


@patch("market_data.Ingest.Ingest._PARAMS")
@patch("market_data.Ingest.SymbolProcessor.process_symbols")
@patch("market_data.Ingest.Summarizer.print_sumary")
@patch("market_data.Ingest.Logger.separator")
def test_perform_single_update(_mock_sep, _mock_summary, mock_process, mock_params):
    ...
    result = Ingester._perform_single_update(["AAPL"], 0.0)
    if result["updated"] != 1:
        raise AssertionError("Expected 1 updated symbol")


@patch("market_data.Ingest.Logger.warning")
def test_load_valid_marketdata_invalid(_mock_warn):
    ...
    result, ts, file_id = Ingester._load_valid_marketdata(invalid_data)
    if result is not None or ts is not None or file_id is not None:
        raise AssertionError(
            "Expected result, timestamp and id to be None for invalid data"
        )


@patch("market_data.Ingest.JsonManager.load")
@patch("market_data.Ingest.JsonManager.save")
@patch("market_data.Ingest.Logger.info")
@patch("market_data.Ingest.Logger.warning")
@patch("market_data.Ingest.GoogleDriveManager.download_file")
@patch("market_data.Ingest.JsonManager.delete")
def test_synchronize_marketdata_only_local(
    ...
    Ingester._synchronize_marketdata_with_drive("mock_path.json")
...
'''
@patch("market_data.Ingest.Updater._perform_single_update")
@patch("market_data.Ingest.RawData.get_latest_price_date")
@patch("market_data.Ingest.RawData.get_stale_symbols")
@patch("market_data.Ingest.Validator.validate_data")
@patch("market_data.Ingest.Ingest._GOOGLE_DRIVE.upload_file")
@patch(
    "market_data.Ingest.Ingest._synchronize_marketdata_with_drive",
    return_value={"drive_data": False},
)
def test_update_data_with_retry(
    ...
    Ingest.update_data(in_max_retries=1, filepath="mock_path.json")
...
def test_load_valid_marketdata_cases(
    ...
    - Valid input with explicit 'last_check' field
    """
    monkeypatch.setattr(
        Updater,
        "is_valid_marketdata",
    ...
    result_data, result_ts, _result_id = Ingester._load_valid_marketdata(input_data)
...
def mock_dependencies():
    ...
    - Retrieving latest price date and stale symbols from RawData.
    ...
    """
    with patch("market_data.Ingest.Ingest._synchronize_marketdata_with_drive"), patch(
        "market_data.Ingest.Updater._perform_single_update"
    ) as mock_update, patch(
        "market_data.Ingest.RawData.get_latest_price_date",
        return_value=pd.Timestamp.now(),
    ), patch(
        "market_data.Ingest.RawData.get_stale_symbols", return_value=[]
    ), patch(
        "market_data.Ingest.Validator.validate_data", return_value=True
    ), patch(
        "market_data.Ingest.Ingest._GOOGLE_DRIVE.upload_file"
    ) as mock_upload:
...
def test_update_data_with_default_filepath_and_retries(mock_dependencies):
    ...
    Ingest.update_data(in_max_retries=2, filepath=" ")
...
def test_sync_drive_newer_than_local(mock_info, mock_save):
    ...
    drive = {
        ...
        "last_check": "2024-02-01T00:00:00Z",
        "symbols": [{"symbol": "AAPL", "historical_prices": [1]}],
    }

    with patch(
        "market_data.Ingest.JsonManager.load", side_effect=[local, drive]
    ), patch(
        "market_data.Ingest.GoogleDriveManager.download_file", return_value=True
    ), patch(
        "market_data.Ingest.JsonManager.delete", return_value=True
    ):
        Ingester._synchronize_marketdata_with_drive("mock_path.json")
...
def test_sync_local_newer_than_drive(mock_info, mock_save):
    ...
    local = {
        ...
        "last_check": "2024-03-01T00:00:00Z",
        "symbols": [{"symbol": "AAPL", "historical_prices": [1]}],
    }
    drive = {
        "last_updated": "2024-02-01T00:00:00Z",
...
def test_sync_drive_only_available(mock_info, mock_save):
    ...
    with patch("market_data.Ingest.JsonManager.load", side_effect=[None, drive]), patch(
        "market_data.Ingest.GoogleDriveManager.download_file", return_value=True
    ), patch("market_data.Ingest.JsonManager.delete", return_value=True):
        Ingester._synchronize_marketdata_with_drive("mock_path.json")
...
def test_sync_none_valid(mock_warn, mock_save):
    ...
    """
    with patch(
        "market_data.Ingest.JsonManager.load", side_effect=Exception("fail")
    ), patch("market_data.Ingest.GoogleDriveManager.download_file", return_value=False):
        Ingester._synchronize_marketdata_with_drive("mock_path.json")
    ...
    if "last_check" not in args[0]:
        raise AssertionError("Expected 'last_check' in saved JSON")
...
def test_both_sources_invalid(
    ...
    Ingester._synchronize_marketdata_with_drive(mock_file_path)
...
def test_drive_newer_than_local(
    mock_info, _mock_delete, mock_download, mock_save, mock_load, mock_file_path
):
    """
...
def test_local_newer_than_drive(
    mock_info, mock_download, mock_save, mock_load, mock_file_path
):
    """
...
@patch("utils.json_manager.JsonManager.load")
@patch("utils.logger.Logger.info")
def test_equal_timestamps_uses_local(mock_info, mock_load, mock_file_path):
    """
...
@patch("utils.json_manager.JsonManager.load")
@patch("utils.logger.Logger.info")
def test_drive_only_available(mock_info, mock_load, mock_file_path):
    """
...
def test_filepath_none_uses_default(monkeypatch):
    ...
    Ingester._synchronize_marketdata_with_drive(None)
    if called["path"] != Ingester._RAW_MARKETDATA_FILEPATH:
...
def test_drive_load_exception(monkeypatch):
    ...
    Ingester._synchronize_marketdata_with_drive("dummy.json")


def test_sync_invalid_default_filepath(monkeypatch):
    """
    Test that _synchronize_marketdata_with_drive raises ValueError when default filepath is invalid.

    Simulates a situation where the internal _RAW_MARKETDATA_FILEPATH is empty,
    and no filepath is provided.
    """
    monkeypatch.setattr(Updater, "_RAW_MARKETDATA_FILEPATH", "  ")
    Ingester._synchronize_marketdata_with_drive(filepath=None)


@patch(
    "market_data.Ingest.EnrichedData.generate",
    side_effect=RuntimeError("test failure"),
)
@patch("market_data.Ingest.Logger.error")
def test_enrich_data_exception(mock_log_error, _mock_generate):
    """
    Test that _enrich_data logs an error when EnrichedData.generate fails.

    Ensures that exceptions during data enrichment are caught and logged.
    """
    Updater.enrich_data()
    if not mock_log_error.called:
        raise AssertionError("Expected Logger.error to be called on exception")
    msg = mock_log_error.call_args[0][0]
    if "Symbol data enrichment failed" not in msg:
        raise AssertionError("Expected error log message for enrichment failure")
