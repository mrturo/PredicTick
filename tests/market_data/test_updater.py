...
def test_load_valid_marketdata_invalid(_mock_warn):
    ...
    result, ts, file_id = Updater._load_valid_marketdata(invalid_data)
...
@patch("market_data.updater.RawData.get_latest_price_date")
@patch("market_data.updater.RawData.get_stale_symbols")
@patch("market_data.updater.Validator.validate_market_data")
@patch("market_data.updater.Updater._GOOGLE_DRIVE.upload_file")
@patch(
    "market_data.updater.Updater._synchronize_marketdata_with_drive",
...
def test_load_valid_marketdata_cases(
    ...
    result_data, result_ts, _result_id = Updater._load_valid_marketdata(input_data)
...
def mock_dependencies():
    ...
    with patch("market_data.updater.Updater._synchronize_marketdata_with_drive"), patch(
        ...
        "market_data.updater.RawData.get_latest_price_date",
        return_value=pd.Timestamp.now(),
    ), patch(
        "market_data.updater.RawData.get_stale_symbols", return_value=[]
    ), patch(
...
def test_both_sources_invalid(
    ...
    Updater._synchronize_marketdata_with_drive(mock_file_path)
...
def test_filepath_none_uses_default(monkeypatch):
    ...
    if called["path"] != Updater._RAW_MARKETDATA_FILEPATH:
...
def test_sync_invalid_default_filepath(monkeypatch):
    ...
    Updater._synchronize_marketdata_with_drive(filepath=None)


@patch(
    "market_data.updater.EnrichedData.generate",
    side_effect=RuntimeError("test failure"),
)
@patch("market_data.updater.Logger.error")
