...
def test_latest_price_date_alias():
    """Confirm latest_price_date alias behaves identically."""
    expected = pd.Timestamp("2025-05-20T15:30:00Z")
    RawData.set_latest_price_date(expected)
    if RawData.latest_price_date() != expected:
        raise AssertionError("latest_price_date alias failed to return expected value.")


def test_invalid_symbols_calls_repo(monkeypatch):
    """Ensure invalid_symbols retrieves from SYMBOL_REPO."""
    monkeypatch.setattr(
        RawData,
        "_SYMBOL_REPO",
        type("DummyRepo", (), {"get_invalid_symbols": lambda _: ["XXX"]})(),
    )
    result = RawData.invalid_symbols()
    if result != {"XXX"}:
        raise AssertionError("invalid_symbols did not return expected set.")
