"""
Unit tests for the Downloader class in the update module.
...
from market_data.ingest.downloader import Downloader
from market_data.ingest.provider import TickerMetadata
...
def test_get_metadata_success(mock_provider, ticker_fixture):
    ...
    if result is None:
        raise AssertionError("Metadata result is None")
...
def test_is_symbol_available_true(mock_get_price):
    ...
    df = pd.DataFrame(
        index=pd.date_range(
            start=now - timedelta(days=1), periods=1, freq="1m", tz="UTC"
        ),
        data={"close": [150.0]},
    )
...
def test_get_historical_prices_single_block(mock_get_price):
    """
    Test get_historical_prices returns non-empty data when the period fits within
    a single block.
    """
    now = datetime.now(timezone.utc)
