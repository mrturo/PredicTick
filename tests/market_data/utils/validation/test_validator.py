...
class TestValidator(unittest.TestCase):
    ...
    def test_datetime_column_not_sorted(self):
        """Should return error if datetime column is not sorted."""
        df = self.valid_data.copy()
        df = df.sample(frac=1).reset_index(drop=True)  # desordena las filas
