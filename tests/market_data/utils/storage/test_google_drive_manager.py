...
def fixture_mock_params():
    ...
    with patch(
        "src.market_data.utils.storage.google_drive_manager.ParameterLoader"
    ) as mock_loader:
...
def fixture_mock_auth_service():
    ...
    with patch("os.path.exists", return_value=True), patch(
        ...
        "src.market_data.utils.storage.google_drive_manager.Credentials.from_authorized_user_file"
    ) as mock_creds, patch(
        "src.market_data.utils.storage.google_drive_manager.build"
    ) as mock_build:
...
def test_upload_new_file(fixture_mock_auth_service):
    ...
    with patch("mimetypes.guess_type", return_value=("application/json", None)), patch(
        "src.market_data.utils.storage.google_drive_manager.MediaFileUpload"
    ):
        manager = GoogleDriveManager(
            service=fixture_mock_auth_service, drive_folder_id="drive_folder_fafe"
        )
        manager.upload_file(_TEST_RAW_MARKETDATA_FILEPATH)
...
def test_upload_with_backup(fixture_mock_auth_service):
    ...
    with patch("mimetypes.guess_type", return_value=("application/json", None)), patch(
        ...
        "src.market_data.utils.storage.google_drive_manager.datetime.datetime"
    ) as mock_dt:
        mock_dt.now.return_value = datetime(2025, 5, 18, 17, 30, 45)
        manager = GoogleDriveManager(
            service=fixture_mock_auth_service, drive_folder_id="drive_folder_fafe"
        )
        manager.upload_file(_TEST_RAW_MARKETDATA_FILEPATH, backup=True)
    create_calls = fixture_mock_auth_service.files.return_value.create.call_args_list
    expected_name = os.path.basename(_TEST_RAW_MARKETDATA_FILEPATH)
    backup_expected_name = (
        Path(_TEST_RAW_MARKETDATA_FILEPATH).stem + "_20250518173045.json"
    )
    ...
    if not any(
        call.kwargs["body"]["name"] == backup_expected_name for call in create_calls
    ):
        pytest.fail(f"Expected backup file upload not found: {backup_expected_name}")
...
def test_authenticate_refresh_token_flow():
    ...
    with patch("os.path.exists", return_value=True), patch(
        ...
        "src.market_data.utils.storage.google_drive_manager.Request"
    ), patch(
        "src.market_data.utils.storage.google_drive_manager.build"
    ) as mock_build:
...
def test_authenticate_installed_app_flow():
    ...
    with patch("os.path.exists", return_value=False), patch(
        "src.market_data.utils.storage.google_drive_manager."
...
def test_file_exists_true(fixture_mock_auth_service):
    ...
    result = manager.file_exists(_TEST_RAW_MARKETDATA_FILEPATH)
    if not result:
...
def test_download_file_success(fixture_mock_auth_service):
    ...
    with patch(
        "src.market_data.utils.storage.google_drive_manager.io.FileIO",
        return_value=MagicMock(),
    ) as _, patch(
        "src.market_data.utils.storage.google_drive_manager.MediaIoBaseDownload"
    ) as mock_downloader:
        ...
        manager.download_file(_TEST_RAW_MARKETDATA_FILEPATH)
...
def test_download_file_not_found(fixture_mock_auth_service):
    ...
    with patch(
        "src.market_data.utils.storage.google_drive_manager.Logger.error"
    ) as mock_log_error:
...
def test_download_file_raises_exception(fixture_mock_auth_service):
    ...
    result = manager.download_file(_TEST_RAW_MARKETDATA_FILEPATH)
...
def test_file_exists_service_none():
    ...
    with patch.object(GoogleDriveManager, "_authenticate", return_value=None):
        ...
        with patch(
            "src.market_data.utils.storage.google_drive_manager.Logger.warning"
        ) as mock_log:
...
def test_download_file_service_none():
    ...
    with patch.object(GoogleDriveManager, "_authenticate", return_value=None):
        ...
        with patch(
            "src.market_data.utils.storage.google_drive_manager.Logger.warning"
        ) as mock_log, patch("os.path.exists", return_value=True):
...
def test_authenticate_raises_exception_logs_warning():
    ...
    with patch(
        "src.market_data.utils.storage.google_drive_manager.os.path.exists",
...
def test_validate_filename_path_object_raises_exception():
    ...
    with patch(
        "src.market_data.utils.storage.google_drive_manager.Path",
