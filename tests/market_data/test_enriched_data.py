...
def test_compute_return_1h(ohlcv_test_data):  # pylint: disable=redefined-outer-name
    """Test compute_return_1h returns 1-hour percent change from close prices."""
    close, *_ = ohlcv_test_data
    result = TechnicalIndicators.compute_return_1h(close)
    if not isinstance(result, pd.Series):
        raise AssertionError("Return 1h must be a Series")


def test_compute_volatility_3h(ohlcv_test_data):  # pylint: disable=redefined-outer-name
    """Test compute_volatility_3h returns 3-hour rolling std dev from return_1h."""
    close, *_ = ohlcv_test_data
    returns = TechnicalIndicators.compute_return_1h(close)
    result = TechnicalIndicators.compute_volatility_3h(returns)
    if not isinstance(result, pd.Series):
        raise AssertionError("Volatility 3h must be a Series")


def test_compute_momentum_3h(ohlcv_test_data):  # pylint: disable=redefined-outer-name
    """Test compute_momentum_3h returns close price difference from 3 hours ago."""
    close, *_ = ohlcv_test_data
    result = TechnicalIndicators.compute_momentum_3h(close)
    if not isinstance(result, pd.Series):
        raise AssertionError("Momentum 3h must be a Series")
