...
class TestDownloader(unittest.TestCase):
    ...
    def test_get_price_data_success_and_filtering(self, _mock_tz):
        """Should return localized, time-filtered DataFrame on first try."""
        # Prepare provider stub
        df = pd.DataFrame(
            {"open": [1, 2, 3]},
            index=pd.to_datetime(["2024-01-01", "2024-01-02", "2024-01-03"], utc=True),
        )
        provider = SimpleNamespace(get_price_data=MagicMock(return_value=df))
        ...
        self.assertEqual(list(out.index), list(df.loc[start:end].index))
        provider.get_price_data.assert_called_once()
        # Ensure PriceDataConfig fields were passed by the Downloader
        args, _kwargs = provider.get_price_data.call_args
        self.assertIsInstance(args[0], PriceDataConfig)
        self.assertEqual(args[0].symbols, "AAPL")
        self.assertEqual(args[0].interval, "1d")
    ...
    def test_get_price_data_retries_and_returns_empty(self, _mock_tz):
        ...
        with patch.object(Downloader, "_PROVIDER", provider), patch(
            ...
            self.assertEqual(provider.get_price_data.call_count, 3)
    ...
    def test_compute_next_valid_datetime_skips_non_trading_days(self, _mock_minutes):
        """Should advance to next session start when day is invalid or after close."""
        ...
        with patch.object(Downloader, "_PARAMS", params), patch(
            ...
            dl = Downloader()
        # Last datetime on excluded day → should jump to Jan 3 at 09:00 UTC
        ...
        # After close on a valid day → next day at open
