...
from market_data.utils.event_dates import EventDates
...
def monkeypatch_json_manager(monkeypatch, mock_class):
    ...
    Args:
        monkeypatch: pytest fixture to apply monkeypatching.
        mock_class: A mock class with static `load` and `exists` methods.
    """
...
def test_get_past_fed_event_days(
    monkeypatch, fed_reference_date
):  # pylint: disable=redefined-outer-name
    """Test retrieving past FED event days with given reference date."""
    monkeypatch_json_manager(monkeypatch, DummyJsonManager)
    ed = EventDates("valid_path.json")
    result = ed.get_past_fed_event_days(fed_reference_date)
    if result != [date(2024, 1, 1)]:
        raise AssertionError("Expected only dates before fed_reference_date")


def test_get_future_fed_event_days(
    monkeypatch, fed_reference_date
):  # pylint: disable=redefined-outer-name
    """Test retrieving future FED event days with given reference date."""
    monkeypatch_json_manager(monkeypatch, DummyJsonManager)
    ed = EventDates("valid_path.json")
    result = ed.get_future_fed_event_days(fed_reference_date)
    if result != [date(2025, 12, 31)]:
        raise AssertionError("Expected only dates after or equal to fed_reference_date")


def test_get_past_future_without_reference(monkeypatch):
    """Test default behavior of past and future methods without a reference date."""
    monkeypatch_json_manager(monkeypatch, DummyJsonManager)
    ed = EventDates("valid_path.json")
    future = ed.get_future_fed_event_days()
    past = ed.get_past_fed_event_days()
    if not all(isinstance(d, date) for d in future + past):
        raise AssertionError("Expected all entries to be date instances")
