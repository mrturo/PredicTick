...
def test_classify_target_periods_none_fallback(monkeypatch):
    """Ensure fallback to 1 when _DEFAULT_PERIODS_AHEAD is None and no param is passed."""
    monkeypatch.setattr(FeatureEngineering, "_DEFAULT_PERIODS_AHEAD", None)

    df = pd.DataFrame(
        {"close": [100, 100, 101]},
        index=pd.date_range("2023-01-03 09:30", periods=3, freq="1min", tz="UTC"),
    )
    result = FeatureEngineering.classify_target(df, threshold=0.005)
    check_eq(result.iloc[0], 1)  # return between 100 and 100 is neutral
    check_eq(result.iloc[1], 2)  # return between 100 and 101 is up
