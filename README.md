...
The repository ships with a hardened multi-stage `Dockerfile` that builds the project virtual environment and runs the update
workflow by default. After installing Docker, you can containerize the project as follows:
...
The image’s default command executes `bash run_tasks.sh update`. Mount any required configuration or environment files (e.g.
`.env`, `config/`) if they are not baked into the image or contain secrets that should stay outside of version control.
